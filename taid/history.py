...
from common import read,write,sha,normalize
from train import load

def main():
    p=argparse.ArgumentParser();p.add_argument('--data',required=True);p.add_argument('--teacher',required=True);p.add_argument('--prompt-cap',type=int,required=True);p.add_argument('--response-cap',type=int,required=True);p.add_argument('--microbatch',type=int,default=1);a=p.parse_args()
    ...
    rows,_=normalize(read(Path(a.data)/'train.json'),a.prompt_cap,a.response_cap,tok.eos_token_id);layers=teacher.model.layers;counts=torch.zeros((teacher.config.vocab_size,len(layers),8),device='cuda',dtype=torch.int64);state={};targets=0
    def hook(layer):
