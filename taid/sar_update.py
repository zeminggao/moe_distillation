...
def update_router(policy, student, optimizer, ids, attention, mask, global_tokens,
                  microbatch=1):
    ...
    for j, (sl, tokens, attn, valid) in enumerate(batches):
        ...
        with torch.autocast('cuda', dtype=torch.bfloat16):
            ...
            weights = torch.ones(len(hidden), device=ids.device, dtype=torch.float32) / global_tokens
